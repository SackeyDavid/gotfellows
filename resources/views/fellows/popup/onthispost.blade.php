                        ...
                        <div class="modal-content">
                          ...
                          <a href="#" class="close" data-dismiss="modal">&times;</a>
                            ...
                            <ul  class="list-unstyled list-inline chats">
                              ...
                              <img  src="{{ URL::to('uploads/images/'.Auth::user()->featured_image) }}" height="45" width="45" data-toggle="tooltip" data-placement="left" 
                              title="
                              <img src='{{ URL::to('uploads/images/'.Auth::user()->featured_image) }}' class='img-responsive' />
                              @if(!$pickings->count())
                              @else
                              ...
                              <button class='btn btn-default' type='submit'>Pick {{ Auth::user()->firstname }} </button> 
                              </a>
                              @endif
                              @endforeach
                              @endif
                              ...
                              {{ Auth::user()->firstname }} posted {{ $post->created_at->diffforhumans() }}
                            ...
                            <li title="post shared from {{ $post->location }}, {{ Auth::user()->country }}"><i class="fa fa-map-marker red" style="color: #7f7f7f"></i> {{ $post->location }} <img src="{{ URL::to('uploads/images/'.Auth::user()->country.'.png') }}" height="17" width="17"  ></li>
                            ...
                            @foreach ($post->comments as $comment)
                                  <li class="by-me">
                                    ...
                                    <div class="avatar pull-left">
                                      <img src="{{ URL::to('uploads/images/'.Auth::user()->featured_image) }}" width="32" height="32" alt="{{ Auth::user()->firstname }}"/>
                                    </div>

                                    <div class="chat-content">
                                      <!-- In meta area, first include "name" and then "time" -->
                                      <div class="chat-meta">{{ Auth::user()->firstname }} <span class="pull-right">{{ $comment->created_at->Diffforhumans() }}</span></div>
