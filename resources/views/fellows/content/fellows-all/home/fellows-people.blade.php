@section('fellows-people')
    ...
    @else
            ...
            <a href="{{ route('user', [ 'name' => $fellow->author['firstname'], 'id' => $fellow->author['id'] ]) }}" class="user-avatar" data-toggle="tooltip" target="_blank" title="<strong>{{ $fellow->name }}</strong> shares a <i>personal</i> credential with you" data-placement="bottom" data-html="true" style="text-decoration: none;">
            <center><strong></strong></center>
              <div class="follow-ava">
              @if($fellow->author['facebook_id'])
                {{ HTML::image($fellow->author['featured_image'], $fellow->author['firstname'], array( 'width' => 32, 'height' => 32 , 'class' => 'img-responsive')) }}
              @else
            	<img alt="" width="32" height="32" src="{{ URL::to('uploads/images/'.$fellow->author['featured_image']) }}" alt="{{ $fellow->author['firstname'] }}" class="img-responsive">
              @endif
                           
              </div>
              </a>
              <p><center style="color:#ccc;">{{ $fellow->description }} </center></p>
              <div class="caption">
